 ...
 * @version Jan 8, 2014 2:50:24 PM
 */

public class UnlockQRTemplate extends AbstractApiCommand implements InitializingBean {

    private static final Logger logger = LoggerFactory.getLogger(UnlockQRTemplate.class);
    ...
    public ApiResult onExecute(ApiCommandContext context) {
        ...
        int QTId = NumberUtils.toInt(stringParams.get("qtId"));
        Object result = null;
        ApiResult apiResult = null;
        // 执行RPC调用
        try {
            long t = System.currentTimeMillis();
            result = templateService.unlockQRTemplate(userId, QTId);
            McpUtils.rpcTimeCost(t, "templateService.unlockQRTemplate");
        } catch (Exception e) {
            // 异常记录日志， 返回错误信息
